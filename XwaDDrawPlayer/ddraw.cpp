...
std::string WStringToString(const std::wstring& wstr)
{
	char texName[256];
	size_t len = 0;
	wcstombs_s(&len, texName, 256, wstr.c_str(), 256);
...
int ExeDDrawLoadTexture_Result(DWORD cbData, PVOID lpData)
{
	...
	//log_debug("[DBG] [XWP] Loading [%s]", WStringToString(name).c_str());
	...
	if (SUCCEEDED(hr))
	{
		...
		log_debug("[DBG] [XWP] Failed when loading [%s]", WStringToString(name).c_str());
